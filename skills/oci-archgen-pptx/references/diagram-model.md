...
- When `external_networks`, `on_premises`, FastConnect, or Site-to-Site VPN is modeled, expect a left-side external network container rendered with the OCI Region container style and labeled as `On-Prem` / `Network` on two lines. Place concrete On-Prem/CPE/resource icons inside that container rather than as loose icons. Do not model or render a duplicate `On-Prem Network` icon inside the container; use that label only as the container heading.
...
- For On-Prem/Customer Data Center/CPE, draw a customer network box to the left of the OCI Region using the same visual family as the Region container. Label the box as `On-Prem` / `Network` on two lines. Place CPE, customer data center, and concrete on-premises resource icons inside the box and keep their labels contained and readable. Do not add a second `On-Prem Network` icon/label inside the box.
