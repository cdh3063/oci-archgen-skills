...
- Always show the ODB@AWS data-plane relationship between each AZ-specific ODB network and its adjacent AWS data center / OCI child site. This connector is required and is separate from the Amazon VPC-to-ODB Network ODB Peering connector.
- Draw the ODB network-to-AWS data center / OCI child-site connector as a short, visible, straight line. Prefer anchoring from the OCI child-site VCN left midpoint when the VCN is shown; otherwise use the nearest AWS data center or OCI child-site edge.
- When this connector is directional, point the arrow from the OCI child-site VCN / AWS data center toward the ODB network.
