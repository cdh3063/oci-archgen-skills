...
- OCI Parents placement is represented as one OCI Parents container for this diagram pattern: even when one AWS Region has ODB@AWS deployed in multiple AZs, render exactly one OCI Parents container.
...
- Show OCI automation/control-plane relationships from OCI Parents / OCI Control Plane to the OCI child site, but avoid making control-plane lines louder than data-plane connectivity.
...
## OCI Parents Rendering

- Render OCI Parents using the standard OCI Region container style, not AWS region styling.
- Use the same visual hierarchy and label treatment as OCI Region rendering in the regular OCI diagram path.
- OCI Parents must be visually separate from the AWS Cloud/AWS Region boundary.
- OCI Parents must use OCI Region/grouping colors, not white or transparent fill. Use a visible gray OCI grouping fill with `#9E9892` line color.
- Center-align the `OCI Parents` label at the top of the OCI Parents container.
- Do not place an OCI icon badge inside the OCI Parents container header.
- Do not place explanatory text such as `Single OCI Parents` or `single-AD based view` inside the diagram body; keep this detail in notes when needed.
- Place OCI control plane inside OCI Parents when it is shown.
- Do not draw one OCI Parents container per AWS AZ. Multiple AWS AZ child-site deployments still share one OCI Parents container in this pattern.
