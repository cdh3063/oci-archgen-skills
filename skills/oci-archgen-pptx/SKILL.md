...
- For on-premises hybrid connectivity, place On-Prem, Customer Data Center/server, and CPE icons inside an external network container to the left of the OCI Region. If the model only names CPE endpoints, still keep a small On-Prem server/DC icon in that container for context. The container should use the same visual family as the OCI Region boundary.
