...
2. Apply OCI architecture guidance.
   ...
   - For actual-state diagrams generated from OCI CLI/resource inventory exports, preserve the discovered resource placement, online/offline state, subnet usage, and naming as the model's source of truth. Do not flag mixed app/db placement, stopped instances, temporary names, broad existing rules, or unused-looking gateways as best-practice deviations unless the user asks for a risk review; focus notes on inventory facts and unknowns.
...
4. Render the PPTX deck.
   ...
   - When on-premises IDC, customer data center, CPE, FastConnect, or VPN connectivity is requested, draw the external customer network as a left-side network container using the same OCI Region container visual treatment. Label that container as `On-Prem` / `Network` on two lines. Keep concrete On-Prem/CPE/service icons inside that container, but do not render a duplicate `On-Prem Network` icon and label inside it.
   ...
   - For OCI CLI/resource inventory diagrams, use actual subnet access labels such as `Public` and `Private` when the subnet contains mixed discovered resources; do not infer `Private-App` or `Private-DB` solely from one resource type.
...
- For on-premises hybrid connectivity, place concrete On-Prem resources, Customer Data Center/server, CPE, and on-premises database icons inside an external network container to the left of the OCI Region. Label the container as `On-Prem` / `Network` on two lines. Do not place an additional `On-Prem Network` icon and label inside the container because the container label already provides that context. If the model only names CPE endpoints, still keep a small On-Prem server/DC icon in that container for context. The container should use the same visual family as the OCI Region boundary.
...
- For OCI CLI/resource inventory diagrams with explicit IPSec VPN resources, show each VPN as its own CPE-to-DRG relationship. When two CPEs connect to one DRG, stack the CPE icons vertically and place the DRG centered between them so the two VPN lines converge in a `>` shape; place the registered `site-to-site-vpn` marker at the DRG-side endpoint of each VPN line, not in the line center. Apply the same VPN marker when only one explicit VPN resource is shown.
