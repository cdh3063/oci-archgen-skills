...
class PptxPackage:
    ...
    def _content_types_xml(self) -> str:
        ...
        if any(Path(media_name).suffix.lower() == ".wmf" for media_name in self._media_bytes):
            media_defaults.append('<Default Extension="wmf" ContentType="image/x-wmf"/>')
        if any(Path(media_name).suffix.lower() == ".emf" for media_name in self._media_bytes):
            media_defaults.append('<Default Extension="emf" ContentType="image/x-emf"/>')
        overrides = [
...
class Renderer:
    ...
    def _add_title_slide(self, slide: SlideBuilder, model: dict[str, Any]) -> None:
        ...
        cover_cloud = self.skill_dir / "assets" / "cover" / "oci-cover-cloud.png"
        if oracle_logo.exists():
            slide.add_picture("Oracle logo", Box(1.12, 1.15, 1.55, 0.32), oracle_logo)
        if cover_cloud.exists():
            slide.add_picture("Cover cloud motif", Box(8.7, 0.42, 3.95, 2.35), cover_cloud)
        slide.add_text(
            "Title",
            Box(1.12, 3.1, 10.0, 0.58),
            title,
            size_pt=31,
            color="312D2A",
            bold=True,
        ...
        slide.add_text(
            "Subtitle",
            Box(1.12, 3.82, 9.2, 0.36),
            str(model.get("subtitle") or "OCI architecture diagram"),
            size_pt=17,
            color="AE562C",
        )
        slide.add_text(
            "Scope",
            Box(1.12, 5.05, 9.2, 0.26),
            scope,
            size_pt=12.5,
            color="312D2A",
            bold=True,
        ...
        slide.add_text(
            "Deck type",
            Box(1.12, 5.38, 9.2, 0.26),
            "Generated architecture deck",
            size_pt=12,
            color="312D2A",
        )
    ...
    def _add_diagram_slide(self, slide: SlideBuilder, model: dict[str, Any]) -> None:
        ...
        if ad_labels:
            ...
            self._draw_container(
                ...
                ad_box,
                str(ad_labels[0]),
                Box(ad_box.x + 0.18, ad_box.y + 0.08, ad_box.w - 0.36, 0.34),
                11.0,
                bold=True,
                align="ctr",
            )
        ...
        self._draw_container(
            ...
            vcn_box,
            vcn_label,
            Box(vcn_box.x + 0.14, vcn_box.y + 0.08, 5.2, 0.34),
            11.0,
            bold=True,
            align="l",
        )
    ...
    def _add_multi_vcn_diagram_slide(self, slide: SlideBuilder, model: dict[str, Any]) -> None:
        ...
        for vcn, region_box, vcn_box, peering_side in layouts:
            self._draw_container(
                ...
                vcn_box,
                self._vcn_label(vcn),
                Box(vcn_box.x + 0.14, vcn_box.y + 0.08, vcn_box.w - 0.28, 0.34),
                11.0,
                bold=True,
                align="l",
            )
    ...
    def _add_notes_slide(self, slide: SlideBuilder, model: dict[str, Any]) -> None:
        ...
        if is_ko:
            ...
            flow_note = "아키텍처 슬라이드에는 모델에 명시된 연결선만 표시했습니다."
        else:
            ...
            flow_note = "Only connection lines explicitly modeled in the request are shown on the architecture slide."
    ...
    def _draw_subnets(
        ...
        for index, subnet in enumerate(subnets):
            ...
            boxes[name] = box
            label = self._subnet_display_label(subnet, f"Subnet {index + 1}")
            self._draw_container(
                slide,
                "subnet",
    ...
    def _subnet_display_label(self, subnet: dict[str, Any], fallback: str) -> str:
        explicit = str(subnet.get("display_name") or subnet.get("display_label") or "").strip()
        if explicit:
            return explicit
        ...
        role = self._private_subnet_role(tier, text)
        if subnet_type == "private" or "private" in text or role:
            return f"Private-{role}" if role else "Private"

        return self._shorten_subnet_name(name) or fallback

    def _private_subnet_role(self, tier: str, text: str) -> str:
        padded = f" {text} "
        ...
        if tier in {"db", "database", "data"} or any(
            ...
            return "DB"
        if tier in {"app", "application", "workload"} or any(
    ...
    def _shorten_subnet_name(self, name: str) -> str:
        ...
        if private_match:
            role = self._private_subnet_role(private_match.group(1), normalized)
            return f"Private-{role}" if role else "Private"
        return name
        ...
        if networks:
            ...
            for index, network in enumerate(visible_networks):
                ...
                self._draw_icon(
                    ...
                    label,
                    icon_box,
                    label_width=1.08 if self._network_is_cpe(network) else max(network_box.w - 0.12, 0.82),
                    label_size=STANDARD_ICON_LABEL_SIZE,
                )
    ...
    def _external_network_center_ys(
        ...
        cpe_indices = [
            index for index, network in enumerate(networks) if self._network_is_cpe(network)
        ]
        if not cpe_indices:
        ...
        step = 0.82
        cpe_midpoint = sum(cpe_indices) / len(cpe_indices)
        centers = [anchor_y + (index - cpe_midpoint) * step for index in range(len(networks))]
    ...
    def _external_networks(self, model: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        if networks and all(self._network_is_cpe(network) for network in networks):
            networks.insert(
                0,
                {
                    "id": "on-prem-network",
                    ...
                    "icon_key": self._canonical_icon_key("customer-data-center") or "customer-data-center",
                },
            )
        return networks
    ...
    def _draw_oracle_service_network(
        ...
        self._draw_container(
            ...
            box,
            "OSN",
            Box(box.x + 0.06, box.y + 0.1, box.w - 0.12, 0.34),
            11.0,
            bold=True,
            align="ctr",
        )
    ...
    def _osn_service_placements(
        ...
        if len(services) < 5:
            icon_size = 0.46 if len(services) >= 3 else STANDARD_ICON_SIZE
            first_center_y = box.y + (0.92 if len(services) >= 3 else 1.24)
            last_center_y = box.bottom - (0.72 if len(services) >= 3 else 1.35)
    ...
    def _oracle_service_network_services(self, model: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        if services:
            ...
            if self._model_has_database_resource(model):
                self._append_osn_service(
                    services,
                    seen,
                    {
    ...
    def _normalize_osn_service(self, item: Any) -> dict[str, Any]:
        if isinstance(item, str):
            ...
            label = self._compact_osn_label(icon_key, item)
        ...
        label = str(item.get("label") or self._compact_osn_label(icon_key, raw))
        result = dict(item)
    ...
    def _compact_osn_label(self, icon_key: str, fallback: Any) -> str:
        normalized = normalize_lookup(icon_key or fallback)
        compact = normalized.replace(" ", "").replace("-", "")
        if compact in {"iam", "identity", "identityandaccessmanagement"}:
            return "IAM"
        if compact in {"audit", "auditing"}:
            return "Audit"
        if compact in {"objectstorage", "buckets"}:
            return "Object Storage"
        return self._icon_label(icon_key) or str(fallback or icon_key)
    ...
    def _model_has_database_resource(self, model: dict[str, Any]) -> bool:
        return any(
            isinstance(resource, dict) and self._is_database_resource(resource)
            for resource in model.get("resources") or []
        )
    ...
    def _draw_gateways(
        ...
        for gateway in gateways:
            ...
            if key == "service-gateway" and osn_box is not None:
                ...
                y = self._gateway_y(key, gateway_type, subnet_boxes, vcn_box, model)
                occupied.append(y)
            elif key == "service-gateway":
                x = vcn_box.right + 0.32
                y = self._avoid_gateway_overlap(
                    self._gateway_y(key, gateway_type, subnet_boxes, vcn_box, model),
                    right_ys,
                    region_box.y + 0.72,
                    region_box.bottom - 0.92,
                )
    ...
    def _draw_resources(
        ...
        self._add_required_public_bastions(model, resources_by_subnet)
        ...
        for subnet_name, subnet_box in subnet_boxes.items():
            ...
            for index, instance in enumerate(expanded):
                ...
                icon_size = self._resource_icon_size(key, cell_w, cell_h)
    ...
    def _add_required_public_bastions(
        self,
        model: dict[str, Any],
        resources_by_subnet: dict[str, list[dict[str, Any]]],
    ) -> None:
        for subnet in self._ordered_subnets(model):
            if not self._subnet_is_public(subnet):
                continue
            subnet_name = str(subnet.get("name") or "")
            if not subnet_name or subnet_name not in resources_by_subnet:
                continue
            resources = resources_by_subnet.setdefault(subnet_name, [])
            if any(self._is_bastion_resource(resource) for resource in resources):
                continue
            resources.insert(
                0,
                {
                    "id": f"auto-bastion-{normalize_lookup(subnet_name).replace(' ', '-')}",
                    "type": "bastion",
                    "label": "Bastion",
                    "subnet": subnet_name,
                    "count": 1,
                    "icon_key": "bastion",
                    "notes": ["Required secure administrative entry point for public subnets."],
                },
            )

    def _subnet_is_public(self, subnet: dict[str, Any]) -> bool:
        value = normalize_lookup(subnet.get("tier") or subnet.get("type"))
        return value in {"public", "edge", "dmz"}

    def _is_bastion_resource(self, resource: dict[str, Any]) -> bool:
        text = normalize_lookup(
            " ".join(
                str(resource.get(field) or "")
    ...
    def _draw_flows(self, slide: SlideBuilder, model: dict[str, Any]) -> None:
        ...
        layout = model.get("layout") or {}
        if not flows and isinstance(layout, dict) and layout.get("synthetic_flows"):
    ...
    def _draw_hybrid_network_connections(self, slide: SlideBuilder, model: dict[str, Any]) -> None:
        ...
        convergence_sources: dict[str, set[str]] = {}
        for source_id, target_id, _label, _dash in specs:
            ...
            drg_id, external_id = self._hybrid_drg_and_external_endpoint(source_id, target_id)
            if drg_id and external_id:
                convergence_sources.setdefault(drg_id, set()).add(external_id)

        convergence_orders: dict[str, dict[str, int]] = {}
        for drg_id, source_ids in convergence_sources.items():
            if len(source_ids) < 2:
                continue
            ordered = sorted(
                source_ids,
                key=lambda source_id: (
                    self._box_for_endpoint(source_id).cy
                    if self._box_for_endpoint(source_id)
                    else 0
                ),
            )
            convergence_orders[drg_id] = {
                source_id: index for index, source_id in enumerate(ordered)
            }
        ...
        for index, (source_id, target_id, label, dash) in enumerate(specs, start=1):
            ...
            drg_id, external_id = self._hybrid_drg_and_external_endpoint(source_id, target_id)
            convergence_order = convergence_orders.get(drg_id or "", {})
            convergence_index = convergence_order.get(external_id or "", 0)
            convergence_total = len(convergence_order)
    ...
    def _hybrid_drg_and_external_endpoint(
        self,
        source_id: str,
        target_id: str,
    ) -> tuple[str | None, str | None]:
        if self._endpoint_is_drg(source_id) and not self._endpoint_is_drg(target_id):
            return normalize_lookup(source_id), normalize_lookup(target_id)
        if self._endpoint_is_drg(target_id) and not self._endpoint_is_drg(source_id):
            return normalize_lookup(target_id), normalize_lookup(source_id)
        return None, None
    ...
    def _hybrid_display_label(self, label: str) -> str:
        ...
        if "fastconnect" in compact:
            return "Fast\nConnect"
        if compact in {"vpn", "ipsec", "ipsecvpn", "sitetositevpn"}:
            return "VPN"
        return label
    ...
    def _draw_hybrid_connection_line(
        ...
        if convergence_total > 1 and drg:
            if target == drg:
                start = (source.right, source.cy)
                end = (drg.x, drg.cy)
            elif source == drg:
                start = (target.right, target.cy)
                end = (drg.x, drg.cy)
            else:
                start = (source.right, source.cy)
            ...
            self._draw_vpn_marker_if_needed(
                slide,
                index,
                label,
                dash,
        ...
        elif drg and self._endpoint_is_drg("drg"):
            lane_y = min(max(source.cy, drg.y + 0.08), drg.bottom - 0.08)
        ...
        self._draw_vpn_marker_if_needed(slide, index, label, dash, start, end, 0, 0)
        self._draw_hybrid_connection_label(slide, index, label, start, end)

    def _draw_vpn_marker_if_needed(
        self,
        slide: SlideBuilder,
        index: int,
        label: str,
        ...
        text = normalize_lookup(label)
        if "vpn" not in text and dash != "dash":
            return
        marker_x = end[0] - 0.10 if end[0] >= start[0] else end[0] + 0.10
        if end[0] >= start[0]:
            marker_x = min(max(marker_x, start[0] + 0.18), end[0] - 0.06)
        else:
            marker_x = max(min(marker_x, start[0] - 0.18), end[0] + 0.06)
        dx = end[0] - start[0]
        ratio = 0.0 if abs(dx) < 0.001 else (marker_x - start[0]) / dx
        marker_y = start[1] + (end[1] - start[1]) * ratio
        if convergence_total > 1:
            marker_y += (convergence_index - (convergence_total - 1) / 2) * 0.12
        self._draw_vpn_lock_marker(slide, f"VPN marker {index}", marker_x, marker_y)

    def _draw_vpn_lock_marker(
        self,
        slide: SlideBuilder,
        name: str,
        cx: float,
        ...
        icon = self._resolve_icon("site-to-site-vpn")
        if icon.asset:
            slide.add_picture(name, Box(cx - 0.143, cy - 0.131, 0.286, 0.262), icon.asset)
            return
        body = Box(cx - 0.12, cy - 0.005, 0.24, 0.16)
        shackle = Box(cx - 0.085, cy - 0.145, 0.17, 0.17)
        keyhole = Box(cx - 0.022, cy + 0.045, 0.044, 0.044)
        slide.add_shape(f"{name} shackle", shackle, None, "2C5967", 1.2, preset="arc")
        slide.add_shape(f"{name} body", body, "FFFFFF", "2C5967", 1.0, preset="roundRect")
        slide.add_shape(f"{name} keyhole", keyhole, "2C5967", "2C5967", 0.5, preset="ellipse")
    ...
    def _draw_hybrid_connection_label(
        ...
        diagonal = abs(start[1] - end[1]) > 0.08
        if diagonal and "vpn" in normalize_lookup(display_label):
            x = line_left + 0.10
        elif "\n" in display_label:
        ...
        y = ((start[1] + end[1]) / 2) - label_h - 0.04
        slide.add_text(
    ...
    def _draw_data_guard_exceptions(self, slide: SlideBuilder, model: dict[str, Any]) -> None:
        ...
        for flow in list(model.get("connections") or []) + list(model.get("flows") or []):
            if not isinstance(flow, dict):
                continue
            label = str(flow.get("label") or flow.get("type") or "")
            if not self._is_data_guard_connection(flow):
                continue
            ...
            if not source or not target:
                continue
            start, end = self._edge_points(source, target)
            display_label = label or "Data Guard"
            slide.add_arrow(
                f"Data Guard connection {source_id} to {target_id}",
                start,
                end,
                arrow=False,
            )
            slide.add_text(
                f"Data Guard label {source_id} to {target_id}",
                self._flow_label_box(start, end, display_label),
                display_label,
                size_pt=CONNECTOR_LABEL_SIZE,
                color=CONNECTOR_LABEL_COLOR,
    ...
    def _is_data_guard_connection(self, connection: dict[str, Any]) -> bool:
        for value in (
            connection.get("type"),
            connection.get("kind"),
            connection.get("label"),
            ...
            normalized = normalize_lookup(value)
            compact = normalized.replace(" ", "").replace("-", "")
            if compact in {"dataguard", "dg", "adg", "activedataguard"}:
                return True
        return False
    ...
    def _draw_icon(
        ...
        display_label = self._display_icon_label(label)
        ...
        if show_external_label:
            label_lines = max(len(display_label.splitlines()), 1)
            label_height = 0.34 if label_lines == 1 else 0.50 + (label_lines - 2) * 0.16
            slide.add_text(
                f"Label {label}",
                Box(icon_box.cx - label_width / 2, icon_box.bottom + 0.02, label_width, label_height),
                display_label,
                size_pt=label_size,
                color="111827",
                align="ctr",
    ...
    def _display_icon_label(self, label: str) -> str:
        raw = str(label or "").strip()
        normalized = normalize_lookup(raw)
        if normalized == "public load balancer":
            return "Public\nLoad Balancer"
        ...
        if mysql_match:
            ...
            heatwave_label = "HeatWave" + (f" {suffix}" if suffix else "")
            return f"MySQL\n{heatwave_label}"
        return raw
    ...
    def _resource_icon_size(self, icon_key: str, cell_w: float, cell_h: float) -> float:
        max_size = min(STANDARD_ICON_SIZE, cell_w * 0.72, cell_h - 0.32)
        return max(min(max_size, STANDARD_ICON_SIZE), 0.38)
    ...
    def _gateway_y(
        ...
        if key == "dynamic-routing-gateway" or gateway_type in {"drg", "dynamic-routing-gateway"}:
            external_center_y = self._hybrid_external_peer_center_y(model)
            if external_center_y is not None:
                return external_center_y - STANDARD_ICON_SIZE / 2
    ...
    def _hybrid_external_peer_center_y(self, model: dict[str, Any] | None) -> float | None:
        if not model:
            return None
        centers: list[float] = []
        for connection in model.get("connections") or []:
            ...
            if not source_id or not target_id:
                continue
            if self._endpoint_refers_to_model_drg(source_id, model):
                peer = self._box_for_endpoint(target_id)
            elif self._endpoint_refers_to_model_drg(target_id, model):
                peer = self._box_for_endpoint(source_id)
            else:
                continue
            if peer is not None:
                centers.append(peer.cy)
        if not centers:
            return None
        return sum(centers) / len(centers)

    def _endpoint_refers_to_model_drg(self, endpoint: str, model: dict[str, Any]) -> bool:
        normalized = normalize_lookup(endpoint)
        if normalized in {"drg", "dynamic routing gateway"}:
            return True
        for gateway in (model.get("vcn") or {}).get("gateways") or []:
            ...
            mapped_key = GATEWAY_ICON_KEYS.get(gateway_type, gateway_type)
            key = self._canonical_icon_key(mapped_key) or mapped_key
            if key != "dynamic-routing-gateway" and gateway_type not in {"drg", "dynamic-routing-gateway"}:
                continue
            values = {
                gateway.get("id"),
                gateway.get("label"),
            ...
            if normalized in {normalize_lookup(value) for value in values if value}:
                return True
        return False
