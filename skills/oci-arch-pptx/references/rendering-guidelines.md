...
- If a single-column subnet stack does not provide enough vertical room and the parent VCN has horizontal room, use a 2 x N subnet layout instead of forcing all subnets into 1 x N.
...
- Use Title Case for English diagram labels and model-visible labels. For example, use `VPC Subnet`, `ODB Network`, `AWS Data Center`, `OCI Child Site`, `OCI Parent Region`, and `OCI Control Plane`.
...
- Render OCI Region and OCI parent region containers with OCI grouping colors from `container-style-map.json`; do not use white or transparent fill for OCI Region boundaries.
- OCI parent region in ODB@AWS diagrams must visually match the OCI Region family, using a gray OCI grouping fill and `#9E9892` line color.
