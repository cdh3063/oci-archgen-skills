...
- OCI parent placement is represented as one OCI parent region for this diagram pattern: even when one AWS Region has ODB@AWS deployed in multiple AZs, render exactly one OCI parent region.
...
- Render the OCI parent region using the standard OCI Region container style, not AWS region styling.
- Use the same visual hierarchy and label treatment as OCI Region rendering in the regular OCI diagram path.
- The OCI parent region must be visually separate from the AWS Cloud/AWS Region boundary.
- The OCI parent region must use OCI Region/grouping colors, not white or transparent fill. Use a visible gray OCI grouping fill with `#9E9892` line color.
- Center-align the `OCI parent region` label at the top of the parent region container.
- Do not place an OCI icon badge inside the OCI parent region container header.
- Do not place explanatory text such as `Single OCI parent region` or `single-AD based view` inside the diagram body; keep this detail in notes when needed.
- Place OCI control plane inside the OCI parent region when it is shown.
- Do not draw one OCI parent region per AWS AZ. Multiple AWS AZ child-site deployments still share one OCI parent region in this pattern.
